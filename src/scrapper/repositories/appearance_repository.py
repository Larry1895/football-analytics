...
def save_or_update_appearance(new_appearance: Appearance):
    ...
    record: Appearance = session.query(Appearance) \
        .filter(Appearance.season_id == new_appearance.season.season_id) \
        .filter(Appearance.player_id == new_appearance.player.player_id) \
        .filter(Appearance.club_id == new_appearance.club.club_id) \
        .filter(Appearance.league_id == new_appearance.league.league_id) \
        .one_or_none()
