...
data_df = pd.DataFrame(data, columns=['avg_value'])
...
# k-means custer
kmeans = KMeans(n_clusters=3, init='k-means++', random_state=0).fit(data_df)
...
fig = px.scatter(data_raw,
                 ...
                 y=column_names[0],  # value
                 color='cluster',
                 text="names")
fig.update_traces(textposition='bottom center', marker_size=20)
fig.update_layout(
    height=720,
    width=1080,
    title_text='k=3',
...
fig.update_yaxes(
    title="Ø Marktwert<br>in Millionen Euro (€)<br>", showgrid=True, showline=True
)
